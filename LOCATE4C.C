/*
locate4.c  Takes in a address number and finds the nearest side street
By............. Andrew Messer
Last Update.... Nov 12, 1992
*/
#include <stdio.h>
#include <string.h>
#include <ctype.h>
...
#define MAX 50                 /* Maxum amout allow in program */
typedef struct
{
	char avenue_name[25];
	int key_number;
}LIST data_list[MAX];   /* Set number of records  */
...
/* Load the stucture file */
void load(void)
{
FILE *fp;
int i;

if((fp=fopen("aveune.dat", "r"))==NULL)
{
	printf("catlog file not on disk \n");
	return;
}
fread(&top, sizeof top, 1, fp);  /* read count */
for(i=0; i<=top; i++)
fread(&data_list[i], sizeof (LIST), 1, fp);
